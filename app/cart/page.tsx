...
export default function CartPage() {
    ...
    try {
      ...
      if (validCoupon) {
        if (
          ...
          setCouponError(
            `This coupon requires a minimum order of ₹${validCoupon.minOrder}`,
          );
          setAppliedCoupon(null);
        } else {
  ...
  const handleCheckout = async () => {
    if (
      ...
      alert(
        "Please fill in your Name, a valid 10-digit Mobile Number, and Address.",
      );
      return;
    }
    if (items.length === 0) {
    ...
    try {
      ...
      await insertOrder({
        ...
        source: "ONLINE",
        items: items.map((i) => {
          const opt = i.product.predefinedOptions?.find(
            (o) => o.label === i.unit,
          );
          return {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FAF9F5] flex items-center justify-center pt-24 md:pt-32">
        <div className="text-[#D4AF37] text-sm tracking-widest uppercase font-bold animate-pulse">
          Loading Cart...
        </div>
      </div>
    );
        ...
        <div className="mb-12">
          <div className="flex items-center gap-4 mb-4">
            ...
            <span className="text-[#D4AF37] text-[10px] font-bold tracking-[0.2em] uppercase">
              {language === "ta" ? "செக்அவுட்" : "Checkout"}
            </span>
          </div>
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#111111] tracking-tight">
        ...
        {items.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-3xl shadow-sm border border-stone-200">
            ...
            <p className="text-stone-500 mb-8">
              {language === "ta"
                ? "உங்கள் கார்ட்டில் இன்னும் எதையும் சேர்க்கவில்லை."
                : "Looks like you haven't added anything to your cart yet."}
            </p>
            <Link
              href="/products"
              className="inline-flex items-center gap-2 rounded-full bg-[#111111] px-8 py-3 text-xs font-bold tracking-widest uppercase text-white hover:bg-[#D4AF37] transition-colors"
            >
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-2/3 flex flex-col gap-6">
              ...
              <div className="bg-white p-4 md:p-6 rounded-3xl shadow-sm border border-stone-200">
                <h2 className="text-xl font-bold text-[#111111] mb-8 pb-4 border-b border-stone-100">
                  {language === "ta" ? "டெலிவரி விவரங்கள்" : "Delivery Details"}
                </h2>
                        ...
                        >
                          <div className="flex flex-row items-center gap-4 flex-1">
                            ...
                            <div className="flex-1">
                              <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#D4AF37] mb-1">
                                {item.product.category}
                              </p>
                              <h4 className="text-xs sm:text-base font-bold text-[#111111] leading-tight">
                                {item.product.name}
                              </h4>
                              <p className="text-[10px] sm:text-xs text-stone-500 font-medium mt-1">
                          ...
                          <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto mt-2 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-0 border-stone-200">
                            <div className="flex items-center gap-2 bg-white rounded-full border border-stone-200 p-1">
                              ...
                              <span className="w-6 text-center text-xs sm:text-sm font-bold">
                                {item.quantity}
                              </span>
                              <button
                                onClick={() =>
            ...
            <div className="w-full lg:w-1/3">
              <div className="bg-[#111111] p-5 md:p-6 rounded-3xl shadow-2xl text-white sticky top-32">
                ...
                <div className="flex flex-col gap-3 mb-8 pb-8 border-b border-white/10">
                  <label className="text-xs font-bold tracking-widest uppercase text-[#D4AF37]">
                    {language === "ta" ? "கூப்பன் குறியீடு" : "Coupon Code"}
                  </label>
                ...
                <div className="flex flex-col gap-4 mb-8">
                  <div className="flex justify-between items-center text-sm font-medium text-white/70">
                    <span>
                      {language === "ta" ? "கூடுதல் தொகை" : "Subtotal"}
                    </span>
                  ...
                  <div className="flex justify-between items-start text-sm">
                    ...
                    <span className="text-red-500 font-bold text-right max-w-[150px] leading-snug">
                      {language === "ta"
                        ? "முகவரியின் அடிப்படையில் கணக்கிடப்பட்டது"
                        : "Calculated based on Address"}
                    </span>
                  </div>
                </div>
